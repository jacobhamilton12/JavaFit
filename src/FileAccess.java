  ...
  public static EnumMap<Config.MuscleGroup, ArrayList<Config.Muscle>> loadFormats() {
    ...
    try {
      ...
      while(scanner.hasNextLine()) {
    	  ...
    	  line.useDelimiter(",");
    	  String group = line.next();
    	  ArrayList<Config.Muscle> muscleList = new ArrayList<Config.Muscle>(); 
    	  while(line.hasNext()) {
    		  muscleList.add(Config.Muscle.valueOf(line.next()));
    	  }
    	  line.close();
    	  retval.put(Config.MuscleGroup.valueOf(group), muscleList);
      }
      scanner.close();
    }
    catch(FileNotFoundException e) {
    	System.out.println("Unable to find workout format file");
    }
	  return retval;
  }
}
